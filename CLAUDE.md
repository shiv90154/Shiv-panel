...
- Tenancy regression (run after any auth/scoping change): `python3 scripts/e2e-tenancy.py http://localhost:3100`. Unit tests: `cd web && npm test` (Node >= 22.18).
- Local admin login at `/login`: admin@test.local (or username `admin`) / supersecretadmin (test env only, from scripts/dc).
...
- `agent/` — host agent (`agent.mjs`, whitelisted RPC); client in `web/src/server/agent.ts`.
- `web/src/lib/` — config, db, session (panel JWT for all roles + encrypted webmail cookie), tenancy (+ pure `tenancy-core`), audit, totp, crypto, password (`{BLF-CRYPT}` $2y$), dkim, dns (record generator + verifier), cloudflare, webmail (IMAP/SMTP), actions-util.
...
- `web/src/app/admin/(panel)/` WHM shell + shared mail pages; `web/src/app/cpanel/` user shell (re-exports the shared pages); `web/src/app/login/`; `web/src/app/actions/auth.ts`; `web/src/app/webmail/`; `web/src/middleware.ts` (host gating, auth redirects, autoconfig).

## Conventions
- Mutations = server actions in `app/**/actions.ts`: `requireSession()`/`requireRole()` first, then load the record with `getOwned*()`/`getManagedAccount()` (tenant scope) BEFORE touching it, wrap body in `run(path, fn)` (`lib/actions-util.ts`) which redirects with `?ok=`/`?error=` (never `redirect()` inside `fn`: `run` catches it; use `run(...).catch()` like `addDomain`). Validate with zod. Write `audit()` for every mutation. List/detail queries use `where: { ...(await scopeFor(session)) }`; new resource tables get `accountId`.
